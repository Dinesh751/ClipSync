...
const Alert = ({ type = 'info', message, onClose, className = '' }) => {
  const getAlertClasses = () => {
    let classes = 'rounded-xl p-4 flex items-start space-x-3 animate-slide-down';
    ...
    <div className={getAlertClasses()}>
      ...
      {onClose && (
        <button
          type="button"
          onClick={onClose}
          className="flex-shrink-0 ml-4 p-1 rounded-lg hover:bg-black/5 transition-colors duration-200"
        >
